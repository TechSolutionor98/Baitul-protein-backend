...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      // Surface validation errors as 400 with readable messages
      if (err.name === "ValidationError") {
        const messages = Object.values(err.errors || {}).map((e) => e.message).filter(Boolean)
        res.status(400)
        throw new Error(messages.length ? messages.join(", ") : "Product validation failed")
      }
      // Duplicate key error (unique fields like slug/sku/barcode)
      if (err.code === 11000) {
        const fields = Object.keys(err.keyPattern || err.keyValue || {})
        res.status(400)
        throw new Error(`Duplicate value for field(s): ${fields.join(", ")}`)
      }
      throw err
    }
  }),
